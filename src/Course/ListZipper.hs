...
--   let (ll, lr) = (break f l) in 
--     if (isEmpty lr) 
--       then IsNotZ
--       else let x:.xs = lr in IsZ (ListZipper xs x ((reverse ll) ++ (pure c) ++ r))
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
-- moveLeftLoop (ListZipper Nil x rs) = let x':.xs' = (reverse (x:.rs)) in (ListZipper xs' x' Nil)
...
moveLeftN' n z | n < 0 =
  moveRightN' (negate n) z
moveLeftN' n (ListZipper Nil _ _) =
  Left n
moveLeftN' n (ListZipper lx x rs) =
  if n > (length lx) then Left (length lx) else moveLeftN' (n - 1) (moveLeftLoop (ListZipper lx x rs))
...
moveRightN' n (ListZipper _ _ Nil) =
  Left n
moveRightN' n (ListZipper lx x rx) =
  if n > (length rx) then Left (length rx) else moveRightN' (n - 1) (moveRightLoop (ListZipper lx x rx))
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n (ListZipper ls x rs) = 
  moveRightN (n - (length ls)) (ListZipper ls x rs)
...
-- prop> \lz -> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
-- end z = let (x:.xs) = reverse (toList z) in ListZipper xs x Nil
end z =
  case moveRight z of
    IsNotZ -> z
    IsZ z' -> end z' 
...
deletePullLeft (ListZipper (x':.ls') _ rs) = IsZ (ListZipper ls' x' rs)
...
deletePullRight (ListZipper ls _ (x':.rs')) = IsZ (ListZipper ls x' rs')
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft n (ListZipper ls x rs) = ListZipper (x:.ls) n rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight n (ListZipper ls x rs) = ListZipper ls n (x:.rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lf f rf) (ListZipper la a ra) =
    ListZipper (zipWith ($) lf la) (f a) (zipWith ($) rf ra)
    -- error "todo: Course.ListZipper (<*>)#instance ListZipper"
...
instance Extend ListZipper where
  (<<=) f z =
    ...
    ListZipper (unfoldr (\z' -> (<$>) (\z'' -> (f z'', z'')) (toOptional (moveLeft z'))) z) (f z) (unfoldr ((<$>) (\z' -> (f z', z')) . toOptional . moveRight) z)
...
instance Traversable ListZipper where
  ...
  traverse f (ListZipper l x r) =
    ...
    lift3 ListZipper (traverse f l) (f x) (traverse f r)
