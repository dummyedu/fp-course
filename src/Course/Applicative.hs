...
instance Applicative List where
  ...
  (<*>) ::
    ...
    -> List a
    -> List b
  -- (<*>) xs ys = flatten $ foldRight (\e acc -> map e ys:.acc) Nil xs
  -- (<*>) xs ys = flatMap (\y -> map ($ y) xs) ys
  (<*>) xs ys = flatMap (\x -> map x ys) xs
...
sequence ::
  ...
  List (f a)
  -> f (List a)
sequence = foldRight (\e acc -> lift2 (\a b -> a:.b) e acc) (pure Nil)
...
replicateA ::
  ...
  -> f a
  -> f (List a)
replicateA c a = sequence (replicate c a)
