...
map ::
  ...
  -> List a
  -> List b
map f xs = foldRight (\e acc -> f e:.acc) Nil xs
...
filter ::
  ...
  -> List a
  -> List a
filter f xs = foldRight (\e acc -> if f e then e:.acc else acc) Nil xs
...
flatten ::
  List (List a)
  -> List a
flatten ll = foldRight (\e acc -> e ++ acc) Nil ll
...
seqOptional lo = foldRight (\e ol -> twiceOptional (:.) e ol) (Full Nil) lo
...
reverse ::
  List a
  -> List a
reverse a = foldLeft (\acc e -> e:.acc) Nil a
