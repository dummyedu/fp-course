...
-- findM p (h :. t) = if (p h) then pure (Full h) else findM p t
...
-- firstRepeat (x1:.left) = if length (filter (== x1) left) > 0 then Full x1 else firstRepeat left
