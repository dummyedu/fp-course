...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok l r c = between (charTok l) (charTok r) c
...
hex ::
  Parser Char
hex = 
  -- (thisMany 4 (satisfy isHexDigit)) >>= \l ->
  --   readHex l
  ...
  in thisMany 4 (satisfy isHexDigit) >>= \l ->
    pure ((chr . hInt) l)
...
satisfyAll ::
  ...
  -> Parser Char
satisfyAll l =
  satisfy (\c -> (foldRight (\e acc -> (e c) && acc) True l))
...
satisfyAny ::
  ...
  -> Parser Char
satisfyAny l =
    satisfy (\c -> (foldRight (\e acc -> (e c) || acc) False l))
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma l r c =
  betweenCharTok l r (sepby c (charTok ','))
